...
def perform_matrix(error,_size,_app):
    ...
    for item in error:
        if item == _app:
            ...
            net_error+=item
        error_total+=item
    print("net acc",1 - net_error/(_net_size*_app))
...
if __name__ == "__main__":
    ...
    #sparsity = 0.8
        ...
        for j in range(100):
            ...
            k=0
            for i in range(1):
                f1 = open("D:/project/CS5/CS8/CS/Viterbi - 副本/synctic_data02"+str(sparsity)+".txt",'a')
                f = open("D:/project/CS5/CS8/CS/Viterbi - 副本/synctic_data02_2"+str(sparsity)+".txt",'a')
                time_start = ti.time()
                #print(y_fix_use)
                #y_fix = y_fix_use
