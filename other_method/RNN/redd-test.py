...
for i in range(5):
    ...
    epochs += 5
    rnn.export_model("D:/project/neural-disaggregator-master/RNN/REDD-RNN-h{}-{}-{}epochs.h5".format(train_building,
                                                        meter_key,
                                                        epochs))
...
print("###########")
for i in range(10):
    disag_filename = 'disag-out'+str(i)+".h5"
    output = HDFDataStore(disag_filename, 'w')
    n=10
    ep = 0.01*(1+i)
    rnn.disaggregate(test_mains, output, train_meter,ep,n, sample_period=sample_period)
    output.close()
    ...
    f = open("D:/project/neural-disaggregator-master/test_real.txt",'a')
    f.write(str(ep)+"\n"+meter_key)
    #temp += (1-rmse)
    input = "============ Recall: {}".format(rpaf[0])+'\n'+"============ Precision: {}".format(rpaf[1])+'\n'+"============ Accuracy: {}".format(rpaf[2])+'\n'
    f.write(input)
    f.close()